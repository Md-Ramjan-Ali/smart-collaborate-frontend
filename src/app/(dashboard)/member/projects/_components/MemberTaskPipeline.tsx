...
interface MemberTaskPipelineProps {
  tasksData: any;
  projectDetails: any;
  auth: any;
      ...
      <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 flex flex-col lg:flex-row gap-4 items-center justify-between shadow-sm">
        <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
          <div className="relative w-full lg:w-48">
            ...
            <input type="text" value={searchVal} onChange={(e) => { setSearchVal(e.target.value); setPage(1); }} placeholder="Search title/desc..." className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg pl-8 pr-3 py-1.5 text-xs text-slate-800 dark:text-slate-200 outline-none w-full focus:border-indigo-500 transition" />
          </div>
        ...
        <table className="w-full text-left text-xs">
          ...
          <tbody>
            {!tasksData?.data?.data || tasksData.data.data.length === 0 ? (
              <tr><td colSpan={5} className="text-center py-10 text-slate-500 font-semibold">No tasks found matching criteria.</td></tr>
            ) : (
                ...
                <tr key={task.id} className="border-b border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-900/30 transition">
                  ...
                  <td className="py-4 text-center">
                    <select
                      ...
                      className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg py-1 px-2.5 text-[11px] font-bold outline-none cursor-pointer text-slate-700 dark:text-slate-300 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      <option value="TO_DO">To Do</option>
                      <option value="IN_PROGRESS">In Progress</option>
                      <option value="UNDER_REVIEW">Under Review</option>
